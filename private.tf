...
module "ec2_private_02" {
  ...
  ec2_key_name = "ashrafelgoharykey"
  user_data = <<-EOF
    #!/bin/bash
    sudo apt update -y
    ...
    sudo echo "<h1>Hello World! - Ashraf Elgohary from private EC2 01</h1>" > /var/www/html/index.nginx-debian.html
...
module "ec2_private_01" {
  ...
  user_data = <<-EOF
    ...
    sudo chmod 777 /var/www/html
    sudo chmod 777 /var/www/html/index.nginx-debian.html
    sudo echo "<h1>Hello World! - Ashraf Elgohary from private EC2 02</h1>" > /var/www/html/index.nginx-debian.html
    sudo systemctl restart nginx
  EOF
}
